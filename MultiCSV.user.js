...
(function() {
  ...
  function inicializarPanel() {
    // Si ya existe el panel, no lo volvemos a crear
    if (document.getElementById('fttx-extractor-panel')) return;

    // Si el body aún no existe por alguna razón, reintentamos en medio segundo
    if (!document.body) {
    ...
    btnEjecutar.addEventListener('click', async () => {
      ...
      actualizarStatus(`Extracción completada.`, "#34d399");
      btnEjecutar.disabled = false;
    });
  }

  // Esperamos 1.5 segundos (1500 ms) antes de inyectar el panel.
  // Esto da tiempo al framework de la web (React/Angular) de construirse
  // sin borrar nuestro panel por accidente.
  setTimeout(inicializarPanel, 1500);
