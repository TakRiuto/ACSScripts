...
(async function() {
    ...
    // Configuración de bucle nativo y precarga
    Object.values(AUDIOS).forEach(a => {
        a.preload = 'auto';
        a.loop = true;
    });

    let sonidoAlerta = AUDIOS.chevette99;
    let silenciado = false;
    let muteGlobal = localStorage.getItem('oltMuteGlobal') === 'true';

    // Desbloqueo de Audio Context por interacción
    let audioAutorizado = false;
    document.addEventListener('click', () => {
        if (!audioAutorizado) {
            audioAutorizado = true;
            // "Cebamos" el audio para que el navegador permita el play automático luego
            Object.values(AUDIOS).forEach(a => {
        ...
        contenedor.innerHTML = logEntradas.map(e => {
            ...
            return `
                <div style="margin-bottom:7px;padding:7px 8px;border-left:4px solid ${colores[e.tipo]};background:rgba(255,255,255,0.04);border-radius:0 4px 4px 0;">
                    ...
                    ${e.datos.zona || e.datos.op ? `<div style="color:#888;font-size:10px;">📍${e.datos.zona} | 🏢${e.datos.op}</div>` : ''}
    ...
    style.innerHTML = `
        @keyframes pulseACS { 0% { box-shadow: inset 0 0 0px #fff; background-color: #a93226; } 50% { box-shadow: inset 0 0 20px #fff; background-color: #ed5565; border:1px solid #fff; } 100% { box-shadow: inset 0 0 0px #fff; background-color: #a93226; } }
        @keyframes pulsePanel { 0% { background-color: rgba(237,85,101,0.1); border-left:5px solid #ed5565; } 50% { background-color: rgba(237,85,101,0.6); border-left:5px solid #fff; } 100% { background-color: rgba(237,85,101,0.1); border-left:5px solid #ed5565; } }
        ...
        .badge-nuevo { background:#fff !important; color:#ed5565 !important; font-size:10px !important; font-weight:900 !important; padding:2px 6px !important; border-radius:4px !important; margin-left:8px !important; box-shadow:0 0 8px #fff; }
        .header-blink { animation:pulsePanel 0.4s infinite alternate !important; box-shadow:0 0 15px #ed5565 !important; }
        .ctrl { background:#222; color:#ed5565; border:1px solid #555; border-radius:3px; padding:3px 5px; font-size:11px; font-weight:bold; cursor:pointer; outline:none; box-sizing:border-box; }
        .ctrl:hover, .ctrl:focus { border-color:#ed5565; }
        #olt-alert-panel.modo-flotante { left: 50% !important; bottom: auto !important; top: 50% !important; transform: translate(-50%, -50%); border-radius: 8px !important; border: 1px solid #555 !important; resize: both; overflow: auto; min-width: 320px; min-height: 200px; }
        #olt-alert-panel.modo-flotante #panel-drag-handle { cursor: move; }
    `;
    ...
    function crearPanel() {
            ...
            <div id="alert-content" style="display:none;">
                ...
                <div id="vista-alarmas">
                    <div style="background:rgba(255,255,255,0.05);padding:8px;margin-bottom:10px;border-radius:4px;display:flex;flex-direction:column;gap:5px;">
                        ...
                        <select id="filtro-op" class="ctrl" style="width:100%;"><option value="TODOS">— Todas —</option></select>
        ...
        tabStyle.innerHTML = `.tab-btn { flex:1; padding:4px 0; font-size:10px; font-weight:bold; border:none; border-radius:3px; cursor:pointer; } .tab-active { background:#ed5565; color:white; } .tab-inactive { background:#333; color:#aaa; }`;
        document.head.appendChild(tabStyle);

        Object.assign(panel.style, { position:'fixed', bottom:'20px', left:'0px', width:'120px', backgroundColor:'rgba(5,5,5,0.98)', color:'white', padding:'12px', borderRadius:'0 8px 8px 0', boxShadow:'5px 0 20px rgba(0,0,0,1)', zIndex:'10000', border:'1px solid #444', borderLeft:'none', transition:'width 0.2s ease' });
        ...
        const MAPA_ALARMA = { placeholder1: 'modo1', alarm: 'alarm', alarmabrazil: 'alarmabrazil', alarmhard: 'alarmhard', chevette99: 'chevette99' };
        document.getElementById('selector-alarma').addEventListener('change', function() {
            const claveAudio = MAPA_ALARMA[this.value];
            sonidoAlerta.pause();
            sonidoAlerta.currentTime = 0;
            sonidoAlerta = AUDIOS[claveAudio];
            const hayAlgoSinLeer = [...registroNodos.values()].some(c => !c.reconocido);
            if (hayAlgoSinLeer && !silenciado && !muteGlobal) sonidoAlerta.play().catch(() => {});
        });

        document.getElementById('filtro-op').addEventListener('change', function() { filtroOp = this.value; });
        document.getElementById('btn-marcar-todos').onclick = () => { for (let data of registroNodos.values()) data.reconocido = true; detenerSonido(); };

        let clicksRapidos = 0, timerClickReset = null, timerMuteTemporal = null, muteExpiraEn = null;
        function aplicarMuteEstado() {
            const statusEl = document.getElementById('mute-status'), btnSil = document.getElementById('btn-silenciar');
            if (!statusEl || !btnSil) return;
            if (muteGlobal) {
                btnSil.style.background = '#7d3c98'; btnSil.style.borderColor = '#9b59b6'; btnSil.style.color = 'white'; statusEl.style.display = 'block'; statusEl.style.color = '#9b59b6'; statusEl.textContent = '🔒 Silencio permanente activo';
            } else if (silenciado && muteExpiraEn) {
                btnSil.style.background = '#784212'; btnSil.style.borderColor = '#e67e22'; btnSil.style.color = 'white'; statusEl.style.display = 'block'; statusEl.style.color = '#e67e22';
                const minRestantes = Math.ceil((muteExpiraEn - Date.now()) / 60000); statusEl.textContent = `⏱ Silenciado ${minRestantes}min restantes`;
            } else {
                btnSil.style.background = '#333'; btnSil.style.borderColor = '#555'; btnSil.style.color = '#ccc'; statusEl.style.display = 'none';
            }
        }
        setInterval(aplicarMuteEstado, 30000);

        document.getElementById('btn-silenciar').addEventListener('click', () => {
            clicksRapidos++; clearTimeout(timerClickReset);
            timerClickReset = setTimeout(() => { clicksRapidos = 0; }, 1000);
            if (clicksRapidos >= 5) {
                clicksRapidos = 0; clearTimeout(timerMuteTemporal); muteExpiraEn = null; muteGlobal = true; silenciado = true;
                localStorage.setItem('oltMuteGlobal', 'true'); detenerSonido(); aplicarMuteEstado(); return;
            }
            if (muteGlobal) { muteGlobal = false; silenciado = false; muteExpiraEn = null; localStorage.setItem('oltMuteGlobal', 'false'); aplicarMuteEstado(); return; }
            if (silenciado && muteExpiraEn) { clearTimeout(timerMuteTemporal); timerMuteTemporal = null; silenciado = false; muteExpiraEn = null; aplicarMuteEstado(); return; }
            const DURACION_MS = 60 * 60 * 1000; muteExpiraEn = Date.now() + DURACION_MS; silenciado = true; detenerSonido();
        ...
        document.getElementById('tab-alarmas').onclick = () => { vistaActual = 'alarmas'; document.getElementById('vista-alarmas').style.display = 'block'; document.getElementById('vista-log').style.display = 'none'; document.getElementById('tab-alarmas').className = 'tab-btn tab-active'; document.getElementById('tab-log').className = 'tab-btn tab-inactive'; };
        document.getElementById('tab-log').onclick = () => { vistaActual = 'log'; document.getElementById('vista-alarmas').style.display = 'none'; document.getElementById('vista-log').style.display = 'block'; document.getElementById('tab-alarmas').className = 'tab-btn tab-inactive'; document.getElementById('tab-log').className = 'tab-btn tab-active'; renderizarLog(); };

        document.getElementById('panel-header').onclick = function(e) {
            if (e.target.id === 'btn-flotante' || modoFlotante) return;
        ...
        document.getElementById('btn-flotante').addEventListener('click', (e) => {
            e.stopPropagation();
            if (document.getElementById('alert-content').style.display === 'none' && !modoFlotante) return;
            modoFlotante = !modoFlotante;
            const btn = document.getElementById('btn-flotante');
            if (modoFlotante) {
                panel.classList.add('modo-flotante'); panel.style.width = '640px'; panel.style.left = '50%'; panel.style.bottom = 'auto'; panel.style.top = '50%'; panel.style.borderLeft = '1px solid #555'; panel.style.borderRadius = '8px'; btn.title = 'Volver'; btn.style.opacity = '1';
                document.getElementById('toggle-btn').style.display = 'none'; document.getElementById('alert-content').style.display = 'block';
            } else {
                panel.classList.remove('modo-flotante'); panel.style.cssText = '';
                Object.assign(panel.style, { position:'fixed', bottom:'20px', left:'0px', width:'300px', backgroundColor:'rgba(5,5,5,0.98)', color:'white', padding:'12px', borderRadius:'0 8px 8px 0', boxShadow:'5px 0 20px rgba(0,0,0,1)', zIndex:'10000', border:'1px solid #444', borderLeft:'none', transition:'width 0.2s ease' });
                document.getElementById('toggle-btn').style.display = 'inline'; document.getElementById('toggle-btn').innerText = '−'; document.getElementById('alert-content').style.display = 'block'; btn.title = 'Flotante'; btn.style.opacity = '0.6';
            }
        });
    }

    function detenerSonido() { sonidoAlerta.pause(); sonidoAlerta.currentTime = 0; }
    ...
    function procesarNodos() {
        ...
        if (oltName !== oltActual) { oltActual = oltName; modoCargaInicial = true; logEntradas = []; registroNodos.clear(); }
        const filas = document.querySelectorAll('tr'), criticosActuales = [], ahora = Date.now();
        let hayNovedadesParaAlarma = false;

        filas.forEach(fila => {
            const celdas = fila.querySelectorAll('td'); if (celdas.length < 2) return;
            const slotStrong = celdas[0].querySelector('strong'); if (!slotStrong) return;
            const slotStr = slotStrong.innerText.trim().padStart(2, '0'); if (!/^\d+$/.test(slotStr)) return;
            for (let i = 1; i < celdas.length; i++) {
                const celdaPort = celdas[i], tituloPuerto = celdaPort.querySelector('.table-cell-head strong')?.innerText.trim() || '';
                const matchPuerto = tituloPuerto.match(/^Port\s+(\d+)$/i); if (!matchPuerto) continue;
                const portStr = matchPuerto[1].padStart(2, '0'), on = parseInt(celdaPort.querySelector('.label-green')?.innerText) || 0, off = parseInt(celdaPort.querySelector('.label-danger')?.innerText) || 0, total = on + off;
                if (total === 0) continue;
                const pDown = ((off / total) * 100).toFixed(1), pUp = ((on / total) * 100).toFixed(1), idNodo = `${oltName}${slotStr}${portStr}A`;
                ...
                if (etiquetas.length > 0) {
                    labelPrincipal = etiquetas[0]; labelPrincipal.innerHTML = `<div style="line-height:1.1;"><b style="font-size:11px;">${pDown}% DN</b><br><span style="font-size:9px;">${pUp}% UP</span></div>`;
                    for (let j = 1; j < etiquetas.length; j++) if (!etiquetas[j].classList.contains('label-success')) etiquetas[j].style.display = 'none';
                }
                if (superaUmbral) {
                    const info = DB_NODOS[idNodo] || { op: "---", zona: "S/I" }, datosNodo = { total, on, off, pDown, pUp, zona: info.zona, op: info.op };
                    if (!registroNodos.has(idNodo)) {
                        registroNodos.set(idNodo, { origen: modoCargaInicial ? 'inicial' : 'nuevo', reconocido: modoCargaInicial, timestamp: ahora, offAnterior: off, pDownAnterior: pDown });
                        if (modoCargaInicial) registrarLog('inicial', idNodo, datosNodo);
                        else { hayNovedadesParaAlarma = true; silenciado = false; registrarLog('nueva_alarma', idNodo, datosNodo); }
                    } else {
                        const data = registroNodos.get(idNodo);
                        if (off > data.offAnterior) { registrarLog('empeora', idNodo, { ...datosNodo, offAntes: data.offAnterior, pDownAntes: data.pDownAnterior }); data.offAnterior = off; data.pDownAnterior = pDown; }
                    }
                    const data = registroNodos.get(idNodo), esNuevoParaPanel = (data.origen === 'nuevo' && !data.reconocido);
                    if (labelPrincipal) {
                        labelPrincipal.className = esNuevoParaPanel ? "label celda-acs-blink" : "label";
                        labelPrincipal.style.cssText = `display:inline-block!important;width:68px!important;color:white!important;border-radius:4px;text-align:center;${!esNuevoParaPanel ? 'background-color:#a93226!important;border:1px solid rgba(255,255,255,0.05);' : ''}`;
                    }
                    criticosActuales.push({ id: idNodo, down: pDown, off, total, ...info, esNuevoParaPanel });
                } else if (labelPrincipal) { labelPrincipal.className = "label"; labelPrincipal.style.cssText = `display:inline-block!important;width:68px!important;background-color:#1ab394!important;color:white!important;border-radius:4px;text-align:center;`; }
            }
        });
        ...
        for (let [id, data] of registroNodos.entries()) if (!idsActivos.has(id)) {
        ...
        if (selectOp) {
            const opsEnOlt = [...new Set(criticosActuales.map(c => c.op).filter(Boolean))].sort(), opcionesActuales = [...selectOp.options].slice(1).map(o => o.value);
            if (JSON.stringify(opsEnOlt) !== JSON.stringify(opcionesActuales)) {
                const selAnterior = selectOp.value; while (selectOp.options.length > 1) selectOp.remove(1);
                opsEnOlt.forEach(op => { const opt = document.createElement('option'); opt.value = op; opt.textContent = op; selectOp.appendChild(opt); });
                selectOp.value = opsEnOlt.includes(selAnterior) ? selAnterior : 'TODOS'; filtroOp = selectOp.value;
            }
        }

        const criticosFiltrados = filtroOp === 'TODOS' ? criticosActuales : criticosActuales.filter(c => c.op === filtroOp), badgeContador = document.getElementById('alert-count'), btnMarcar = document.getElementById('btn-marcar-todos'), hayAlgoSinLeer = criticosActuales.some(c => c.esNuevoParaPanel);
        ...
        if (listContainer) {
            const nuevoHTML = criticosFiltrados.length > 0 ? criticosFiltrados.map(c => `<div class="${c.esNuevoParaPanel ? 'tarjeta-panel-blink' : ''}" style="margin-bottom:10px;padding:9px;border-left:5px solid #ed5565;background:rgba(255,255,255,0.05);border-radius:0 5px 5px 0;"><div style="display:flex;align-items:center;justify-content:space-between;"><span style="color:#1ab394;font-weight:900;font-size:14px;letter-spacing:0.5px;">${c.id}</span>${c.esNuevoParaPanel ? '<span class="badge-nuevo">NUEVO</span>' : ''}</div><div style="font-size:10px;color:#ddd;margin:3px 0;">📍 ${c.zona} | 🏢 ${c.op}</div><div style="margin-top:5px;color:#ed5565;font-size:11px;font-weight:bold;">⚠️ ${c.down}% caída | 🔴 OFF:${c.off} | 👥 ${c.total}</div></div>`).join('') : (criticosActuales.length > 0 ? `<div style="color:#aaa;text-align:center;padding:20px;font-size:11px;">Sin alarmas para <b>${filtroOp}</b></div>` : '<div style="color:#1ab394;text-align:center;padding:20px;font-weight:bold;">SISTEMA OK ✅</div>');
    ...
    const worker = new Worker(URL.createObjectURL(new Blob([`setInterval(()=>postMessage('tick'),2500)`], { type:'application/javascript' })));
